...
foodshop <- read.csv("incheon_food_2023.csv", na="", stringsAsFactors = F)
...
#5.address변수
foodshop$district<-substr(foodshop$address,6,9)#구 정보를 분리하여 변수 생성
table(foodshop$district)#이상치 확인
foodshop$district <- ifelse(foodshop$district%in%c("남시 중","주시 광"),NA,foodshop$district)#이상치제거
table(foodshop$district)#이상치 확인
